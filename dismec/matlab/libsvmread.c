...
void read_problem(const char *filename, long long nlhs, mxArray *plhs[])
{
	long long max_index, min_index, inst_max_index, i;
	long elements, k;
	...
	while(readline(fp) != NULL)
	{
		...
		while (1)
		{
			...
			errno = 0;
			index = (long long) strtol(idx,&endptr,10);
			if(endptr == idx || errno != 0 || *endptr != '\0' || index <= inst_max_index)
			{
				mexPrintf("Wrong input format at line %ld\n",l+1);
				fake_answer(nlhs, plhs);
				return;
			}
			else
	...
	for(i=0;i<l;i++)
	{
		...
		if(label == NULL)
		{
			mexPrintf("Empty line at line %ld\n",i+1);
			fake_answer(nlhs, plhs);
			return;
		}
