...
long long (*info)(const char *fmt,...);

long long col_format_flag;

void read_sparse_instance(const mxArray *prhs, long long index, struct feature_node *x, long long feature_number, double bias)
{
	long long i, j, low, high;
	mwIndex *ir, *jc;
	double *samples;
	...
	j = 0;
	low = (long long) jc[index], high = (long long) jc[index+1];
	for(i=low; i<high && (long long) (ir[i])<feature_number; i++)
	{
		x[j].index = (long long) ir[i]+1;
		x[j].value = samples[i];
		j++;
	}
	if(bias>=0)
...
static void fake_answer(long long nlhs, mxArray *plhs[])
{
	long long i;
	for(i=0;i<nlhs;i++)
...
void do_predict(long long nlhs, mxArray *plhs[], const mxArray *prhs[], struct model *model_, const long long predict_probability_flag)
{
	long long label_vector_row_num, label_vector_col_num;
	long long feature_number, testing_instance_number;
	long long instance_index;
	...
	testing_instance_number = (long long) mxGetM(prhs[1]);
	if(col_format_flag)
	{
		feature_number = (long long) mxGetM(prhs[1]);
		testing_instance_number = (long long) mxGetN(prhs[1]);
	}

	label_vector_row_num = (long long) mxGetM(prhs[0]);
	label_vector_col_num = (long long) mxGetN(prhs[0]);
...
{
	long long prob_estimate_flag = 0;
	struct model *model_;
	char cmd[CMD_LEN];
	info = &mexPrintf;
	col_format_flag = 0;
