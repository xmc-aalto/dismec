...
const char *matlab_matrix_to_model(struct model *model_, const mxArray *matlab_struct)
{
	...
	// Parameters
	ptr = mxGetPr(rhs[id]);
	model_->param.solver_type = (long long)ptr[0];
	id++;

	// nr_class
	ptr = mxGetPr(rhs[id]);
	model_->nr_class = (long long)ptr[0];
	id++;
	...
	// nr_feature
	ptr = mxGetPr(rhs[id]);
	model_->nr_feature = (long long)ptr[0];
	id++;

	// bias
	ptr = mxGetPr(rhs[id]);
	model_->bias = (long long)ptr[0];
	id++;
	...
	{
		model_->label = Malloc(long long, model_->nr_class);
		ptr = mxGetPr(rhs[id]);
		for(i=0;i<model_->nr_class;i++)
			model_->label[i] = (long long)ptr[i];
	}
	id++;
